...
to be added...
In the concerto system you need to set up a few variables and know the location where you want to store any graphs your code produces
The command for histograms specifically is called hist(x, breaks, main, col, xaxt) and it uses input parameters to configure the look of the histogram. You can pass a histogram into a data type that can return values about the graph for debugging for example you can have
